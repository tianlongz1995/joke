...
<div class="box-inner">
	<div class="box-header well" data-original-title="">
		<h2><i class="glyphicon glyphicon-user"></i> 渠道月报</h2>
	</div>
		...
		<table  class="table table-striped table-bordered bootstrap-datatable responsive">
				...
				<div class="col-md-12">
					<div class="dataTables_filter" id="DataTables_Table_0_filter">
						<label style="padding-right:30px;">
							<span >开始月</span>
							<c:if test="${empty startMonth}">
								<input type="text" id="startMonth" onfocus="WdatePicker({dateFmt:'yyyyMM', isShowToday: false, isShowClear: false })" class="Wdate" value=""/>
							</c:if>
							<c:if test="${!empty startMonth}">
								<input type="text" id="startMonth" onfocus="WdatePicker({dateFmt:'yyyyMM', isShowToday: false, isShowClear: false })" class="Wdate" value="${startMonth}"/>
							</c:if>
						</label>
						<label style="padding-right:30px;">
							<span >结束月</span>
							<c:if test="${empty endMonth}">
								<input type="text" id="endMonth" onfocus="WdatePicker({dateFmt:'yyyyMM', isShowToday: false, isShowClear: false })" class="Wdate" value=""/>
							</c:if>
							<c:if test="${!empty endMonth}">
								<input type="text" id="endMonth" onfocus="WdatePicker({dateFmt:'yyyyMM', isShowToday: false, isShowClear: false })" class="Wdate" value="${endMonth}"/>
							</c:if>
						</label>
						<label style="padding-right:30px;">
							<span >渠道名称</span>
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<c:if test="${timeDetail.totalUv == 0 }">0%</c:if>
						<c:if test="${timeDetail.totalUv != 0 }">
					...
					<td>
						<c:if test="${timeDetail.enterUv == 0 }">0%</c:if>
						<c:if test="${timeDetail.enterUv != 0 }">
					...
					<td>
						<c:if test="${timeDetail.listUv == 0 }">0%</c:if>
						<c:if test="${timeDetail.listUv != 0 }">
					...
					<td>
						<c:if test="${timeDetail.detailUv == 0 }">0%</c:if>
						<c:if test="${timeDetail.detailUv != 0 }">
...
<script type="text/javascript">
	function turnPage(){
		location.href = '<%=basePath%>statistics/monthDetail?startMonth='+$("#startMonth").val()+'&endMonth='+$("#endMonth").val()+
