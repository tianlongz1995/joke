        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(commentList)) {
                for (Comment comment : commentList) {
                    ...
                    jedisCache.zadd(commentListKey, comment.getCreateTime(), String.valueOf(comment.getId()));
                    if (comment.getGood() != null && comment.getGood() >= 10) {//神评缓存
                ...
                //更新已发布评论状态
                commentService.updateCommentState(ids.deleteCharAt(ids.lastIndexOf(",")).toString(), 4);
